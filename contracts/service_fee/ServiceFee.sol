...
import "../interfaces/ILoopXToken.sol";

/**
 * @notice Service Fee contract for LoopX NFT Marketplace
 */
contract ServiceFee is AccessControl {
    using Address for address;

    /// @notice service fee contract
    ILoopXToken public loopxTokenContract;
    ...
    event LoopXTokenContractUpdated(address loopxTokenContract);
    ...
    */
    ...
    /**
     * @notice Lets admin set the LoopX token contract
     * @param _loopxTokenContract address of loopx token contract
     */
    function setLoopXTokenContract(address _loopxTokenContract) onlyAdmin external {
        require(
            _loopxTokenContract != address(0),
            "ServiceFee.setLoopXTokenContract: Zero address"
        );
        loopxTokenContract = ILoopXToken(_loopxTokenContract);
        emit LoopXTokenContractUpdated(_loopxTokenContract);
    }
    ...
    function getSellServiceFeeBps(address _seller, bool _isSecondarySale) external view onlyProxy returns (uint256) {
        ...
        uint256 balance = loopxTokenContract.balanceOf(_seller);
    ...
    function getBuyServiceFeeBps(address _buyer) onlyProxy external view returns (uint256) {
        ...
        uint256 balance = loopxTokenContract.balanceOf(_buyer);
