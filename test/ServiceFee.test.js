...
contract("ServiceFee", (accounts) => {
    ...
    const mint = async (minter, tokenId, fees = []) => {
        ...
        await nitaerc721token_contract.mint(
            tokenId,
    ...
    before(async () => {
        await NitaERC721Token.new(
            ...
            nitaerc721token_contract = instance;
        });

        await NitaToken.new({ from: accounts[1] }).then(function (instance) {
            nitatoken_contract = instance;
        });
        ...
        await servicefee_contract.setNitaTokenContract(nitatoken_contract.address, { from: accounts[0] })
    ...
    describe("calculate fee", () => {
        it("works fine with existing service fee contract", async () => {
            ...
            await nitaerc721token_contract.setApprovalForAll(
                transferproxy_contract.address,
                true,
                { from: accounts[6] }
            );
            ...
            const saleContractBuySignature = soliditySha3(
                nitaerc721token_contract.address, // token
                tokenId, // tokenId
            ...
            await sale_contract.setApproveSaleDate(nitaerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[6] })
            ...
            await sale_contract.buy(
                nitaerc721token_contract.address, //IERC721 token,
                tokenId, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (NitaERC721Token)
    ...
    describe("Buyer Service Fee", () => {
        ...
        it("The buyer service fee is 225 bps if holds more than 100 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[9], new BN('100000000000000000000'), {from: accounts[1]});
            const buyerServiceFeeBps = new BN(await servicefeeproxy_contract.getBuyServiceFeeBps(accounts[9]))
            assert.equal(buyerServiceFeeBps.toString(), '225');
        })
        it("The buyer service fee is 200 bps if holds more than 1000 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[9], new BN('1000000000000000000000'), {from: accounts[1]});
            const buyerServiceFeeBps = new BN(await servicefeeproxy_contract.getBuyServiceFeeBps(accounts[9]))
            assert.equal(buyerServiceFeeBps.toString(), '200');
        })
        it("The buyer service fee is 175 bps if holds more than 2500 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[9], new BN('2500000000000000000000'), {from: accounts[1]});
            const buyerServiceFeeBps = new BN(await servicefeeproxy_contract.getBuyServiceFeeBps(accounts[9]))
            assert.equal(buyerServiceFeeBps.toString(), '175');
        })
        it("The buyer service fee is 150 bps if holds more than 10000 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[9], new BN('10000000000000000000000'), {from: accounts[1]});
            const buyerServiceFeeBps = new BN(await servicefeeproxy_contract.getBuyServiceFeeBps(accounts[9]))
    ...
    describe("Sell Primary Service Fee", () => {
        ...
        it("The seller service fee is 225 bps if holds more than 100 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[7], new BN('100000000000000000000'), {from: accounts[1]});
            const sellerServiceFeeBps = new BN(await servicefeeproxy_contract.getSellServiceFeeBps(accounts[7], false))
            assert.equal(sellerServiceFeeBps.toString(), '225');
        })
        it("The seller service fee is 200 bps if holds more than 1000 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[7], new BN('1000000000000000000000'), {from: accounts[1]});
            const sellerServiceFeeBps = new BN(await servicefeeproxy_contract.getSellServiceFeeBps(accounts[7], false))
    ...
    describe("Sell Secondary Service Fee", () => {
        ...
        it("The seller service fee is 225 bps if holds more than 100 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[8], new BN('100000000000000000000'), {from: accounts[1]});
            const sellerServiceFeeBps = new BN(await servicefeeproxy_contract.getSellServiceFeeBps(accounts[8], true))
            assert.equal(sellerServiceFeeBps.toString(), '225');
        })
        it("The seller service fee is 200 bps if holds more than 10,00 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[8], new BN('1000000000000000000000'), {from: accounts[1]});
            const sellerServiceFeeBps = new BN(await servicefeeproxy_contract.getSellServiceFeeBps(accounts[8], true))
            assert.equal(sellerServiceFeeBps.toString(), '200');
        })
        it("The seller service fee is 175 bps if holds more than 2500 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[8], new BN('2500000000000000000000'), {from: accounts[1]});
            const sellerServiceFeeBps = new BN(await servicefeeproxy_contract.getSellServiceFeeBps(accounts[8], true))
            assert.equal(sellerServiceFeeBps.toString(), '175');
        })
        it("The seller service fee is 175 bps if holds more than 10000 of nita tokens.", async () => {
            await nitatoken_contract.transfer(accounts[8], new BN('10000000000000000000000'), {from: accounts[1]});
            const sellerServiceFeeBps = new BN(await servicefeeproxy_contract.getSellServiceFeeBps(accounts[8], true))
