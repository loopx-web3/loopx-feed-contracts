...
contract("ERC721Sale", (accounts) => {
    ...
    before(async () => {
        ...
        await servicefee_contract.setNitaTokenContract(nitatoken_contract.address, { from: accounts[1] })
    ...
    describe("buy", () => {
        it("works and costs something (minter:6)", async () => {
            ...
            // Owner changed
            const newOwner = await nitaerc721token_contract.ownerOf(tokenId);
        ...
        it("fails without minted token owner approval (minter:3)", async () => {
            ...
            // The owner of the token has to approve the transferproxy for sale == settings something on sale
            // DISABLED TO TEST --> await nitaerc721token_contract.setApprovalForAll(transferproxy_contract.address, true, {from: accounts[3]});
            ...
            await sale_contract.setApproveSaleDate(nitaerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[3] })
        ...
        it("ApproveAll only needed once per minter (minter:4)", async () => {
            ...
            await sale_contract.setApproveSaleDate(nitaerc721token_contract.address, tokenId1, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[4] })

            // Sold one token
            await sale_contract.buy(
                nitaerc721token_contract.address, //IERC721 token,
                tokenId1, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (NitaERC721Token)
            ...
            const newOwner = await nitaerc721token_contract.ownerOf(tokenId1);
            assert.equal(accounts[7], newOwner);

            await sale_contract.setApproveSaleDate(nitaerc721token_contract.address, tokenId2, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[4] })

            // Sold two tokens
            await sale_contract.buy(
                nitaerc721token_contract.address, //IERC721 token,
                tokenId2, //uint256 tokenId HAS TO BE THE PASSED TOKEN ID OF A MINTED TOKEN (NitaERC721Token)
                accounts[4], //address payable owner, the owner of the token from who we are buying the token from
                saleApprovalSignature2.signature, //signtaure
                { from: accounts[8], value: messageValue }
            );

            const newOwner2 = await nitaerc721token_contract.ownerOf(tokenId2);
        ...
        it("does not work if prev owner try to approve", async () => {
            ...
            // Owner changed
            let newOwner = await nitaerc721token_contract.ownerOf(tokenId);
            assert.equal(accounts[7], newOwner);
        ...
        it("can't buy token again once it was bought.", async () => {
            ...
            let thrownError;
            try {
                await sale_contract.setApproveSaleDate(nitaerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-03-31')), { from: accounts[8] })
            } catch (error) {
                thrownError = error;
            }
        ...
        it("can't set sale date if not owner", async () => {
            ...
            try {
                await sale_contract.setApproveSaleDate(nitaerc721token_contract.address, tokenId, getUnixEpochTimeStamp(new Date('2021-05-30')), { from: accounts[5] })
            } catch (error) {
                thrownError = error;
            }
